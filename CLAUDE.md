...
**Próximo:** com os três de-risks de cliente do F0 fechados (faixa, toasts, taskbar, sobre WPF/ADR-001) e os docs de fundação sincronizados, duas frentes destravam — escolher no **board vivo** (`h1ve start`; o board é a fila, **não** o roadmap, ritual SPEC-176): **(1) Trilha GTM — G.1 briefing de identidade visual** (destrava o relógio da wishlist na Steam: nome + capsule no ar); **(2) Fase 0.2 — camada de dados + seed do mundo** (persistir o que o `world-engine` provou em memória, já ciente da **Pirâmide Elástica** / grupos paralelos). Depois, a trilha server-first: 0.3 RNG/auditoria → 0.4 segurança. *(A antiga “Landing waiting list” saiu de cena — o Steam-only substituiu a landing própria pela página Coming Soon na Steam, SPEC-007.)*
