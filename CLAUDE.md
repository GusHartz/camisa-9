...
**Próximo:** com o F0 técnico fechado, o R4 FINAL sincronizado nos docs (SPEC-011) e o **elenco 16 já aplicado no engine (SPEC-012)**, as frentes destravam — escolher no **board vivo** (`h1ve start`; o board é a fila, **não** o roadmap, ritual SPEC-176): **(1) Fase 0.2 — camada de dados + seed do mundo** (persistir o `WorldState` que a SPEC-009 provou em memória, já com o **elenco 16** e ciente da **Pirâmide Elástica** / grupos paralelos — **card a criar no board**); **(2) spec de rodadas diárias (1.2)** com o encaixe da Copa (card `Rodadas ter/qui/sáb 15h` a **renomear** para a cadência diária); **(3) Trilha GTM — G.1 briefing de identidade visual** (destrava o relógio da wishlist na Steam — **card a criar**). Depois, a trilha server-first: 0.3 RNG/auditoria → 0.4 segurança. *(Higiene do board pendente: renomear os 4 cards da cadência antiga; criar os cards de 0.2 e G.1 — ação de founder na UI H1VE.)*
