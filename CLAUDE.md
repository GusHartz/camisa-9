...
**Próximo:** com a **Fatia 1 da 0.2 entregue** (o `WorldState` já persiste em Postgres, snapshot reconciliado com o golden), a fila natural é a **Fatia 2 — publicador de rodada transacional** (a joia do money path: portar o `RoundStore`/`RoundPublisher` da SPEC-002 do `Set` in-process para Postgres real — `BEGIN`/stage/`COMMIT` numa tx, `UNIQUE(league_id,season_id,round)` como idempotência, advisory lock no lugar do lock in-process). Escolher sempre no **board vivo** (`h1ve start`; o board é a fila, **não** o roadmap, ritual SPEC-176). Outras frentes abertas: **(a) spec de rodadas diárias (1.2)** com o encaixe da Copa (card `Rodadas ter/qui/sáb 15h` a **renomear** para a cadência diária); **(b) Trilha GTM — G.1 briefing de identidade visual** (destrava o relógio da wishlist na Steam — **card a criar**). Depois, a trilha server-first: 0.3 RNG/auditoria → 0.4 segurança. *(Higiene do board pendente: renomear os 4 cards da cadência antiga; criar o card de G.1 — ação de founder na UI H1VE.)*
