...
**Próximo:** com a **Fatia 2 entregue** (a publicação de rodada agora é atômica e durável em Postgres — o money path deixou de ser só in-memory), a fila natural passa a ser **spec de rodadas diárias (1.2)** — o job que **chama** o `publishRound` todo dia às 15h (card `Rodadas ter/qui/sáb 15h` a **renomear** para a cadência diária 7/7; o engine ainda crava `MATCH_DAYS=[2,4,6]` em `anchor.ts`, a virar junto). Escolher sempre no **board vivo** (`h1ve start`; o board é a fila, **não** o roadmap, ritual SPEC-176). Outras frentes abertas: as **Fatias 3-5 da 0.2** (viragens persistidas → Neon branch-por-ambiente → Pirâmide Elástica — **cards a criar**); **Trilha GTM — G.1 briefing de identidade visual** (destrava o relógio da wishlist na Steam — **card a criar**). Depois, a trilha server-first: 0.3 RNG/auditoria → 0.4 segurança. *(Higiene do board pendente: renomear os cards da cadência antiga; criar os cards das Fatias 3-5 e de G.1 — ação de founder na UI H1VE.)*
