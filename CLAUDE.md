...
> Atualizado ao final de cada sessão. Última atualização: **2026-07-15** (SPEC-005).
...
**Próximo:** escolher no **board vivo** (`h1ve start`) — o board é a fila de trabalho, **não** o roadmap (ritual SPEC-176). Com o **#3 (toasts) validado**, os spikes de cliente sobre a stack WPF (ADR-001) que restam iniciáveis: *Landing waiting list*, *Spike widget na taskbar* (#4). A trilha server-first do roadmap (0.2 Camada de dados → 0.3 RNG/auditoria → 0.4 segurança) segue como **direção de longo prazo**, entrando na fila quando priorizada no board.
