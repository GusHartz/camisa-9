...
> Atualizado ao final de cada sessão. Última atualização: **2026-07-15** (SPEC-006).
...
**Próximo:** escolher no **board vivo** (`h1ve start`) — o board é a fila de trabalho, **não** o roadmap (ritual SPEC-176). Com o **#3 (toasts) e #4 (widget/taskbar) validados** (os três de-risks de cliente do F0 — faixa, toasts, taskbar — fechados sobre a stack WPF/ADR-001), resta iniciável de cliente: *Landing waiting list*. A trilha server-first do roadmap (0.2 Camada de dados → 0.3 RNG/auditoria → 0.4 segurança) segue como **direção de longo prazo**, entrando na fila quando priorizada no board.
