...
/// <summary>
/// Calcula o retângulo (px físico) da faixa compacta ancorada à taskbar, para a postura
/// topmost. `ABM_GETTASKBARPOS` dá a taskbar do sistema; `GetMonitorInfo(rcWork)` dá a área
/// útil (já sem a taskbar) do monitor da janela. Política de UI (OP-17).
/// </summary>
internal static class TaskbarAnchor
{
    public enum Edge { Left, Top, Right, Bottom, Unknown }
    ...
    {
        if (tb.Right - tb.Left == 0 && tb.Bottom - tb.Top == 0) return Edge.Bottom; // fallback
        if (tb.Width >= tb.Height) return tb.Top <= mon.Top ? Edge.Top : Edge.Bottom;
    ...
    private static RECT BandRect(RECT work, Edge edge, int w, int h) => edge switch
    {
