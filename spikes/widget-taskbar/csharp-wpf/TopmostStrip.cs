...
/// oculto) e detecta o cloaking do Win+D. Política de UI (OP-17).
/// </summary>
internal static class TopmostStrip
{
    /// <summary>WS_EX_NOACTIVATE|TOOLWINDOW — não rouba foco, fora do Alt-Tab (as duas posturas).</summary>
    ...
    public static void Reassert(IntPtr hwnd) =>
        NativeMethods.SetWindowPos(hwnd, Win.HWND_TOPMOST, 0, 0, 0, 0,
            Win.SWP_NOMOVE | Win.SWP_NOSIZE | Win.SWP_NOACTIVATE);

    /// <summary>Win+D / mostrar desktop: o shell "cloaka" a janela via DWM (gap da SPEC-003).</summary>
    public static bool IsCloaked(IntPtr hwnd)
    {
        int hr = NativeMethods.DwmGetWindowAttribute(hwnd, Win.DWMWA_CLOAKED, out int cloaked, sizeof(int));
