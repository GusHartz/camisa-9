...
.SYNOPSIS
  ...
  Spike SPEC-005 — o app-gatilho idle deve ficar em ~0% CPU e sem leak de objetos de
  notificação acumulados. Copiado da SPEC-003 (parametrizado, locale-independent).

.EXAMPLE
  # App-gatilho aberto e ocioso (2 min):
...
.NOTES
  ...
  RAM = WorkingSet64 (working set TOTAL). Drift de RAM = fim − início; positivo e persistente = leak.
  Obs.: no fluxo de produção o cliente cold-starta por clique e sai — aqui medimos o
  app-gatilho ABERTO ocioso (pior caso de residência) e o footprint publicado (ver README).
#>
param(
  [string]$ProcessName = "WidgetTaskbar",
  [int]$Seconds = 120,
