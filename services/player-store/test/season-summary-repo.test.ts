...
describe.skipIf(!DB_URL)('season-summary-repo — a campanha da temporada contra Postgres real', () => {
  let handle: DbHandle;
  ...
  it('acumula ao longo da temporada e guarda o RECORDE com a rodada dele', async () => {
    const { athleteId } = await newAthlete();
    await accrueSeasonMatch(handle.db, athleteId, match({ round: 1, day: 1, rating: 64, goals: 0 }));
    await accrueSeasonMatch(handle.db, athleteId, match({ round: 2, day: 2, rating: 83, goals: 2 }));
    await accrueSeasonMatch(handle.db, athleteId, match({ round: 3, day: 3, rating: 71, goals: 1 }));
