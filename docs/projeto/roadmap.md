...
> **De-risk do cliente (paralelo, fora da sequência numerada) — SPEC-003 ✅ validado (2026-07-15).** Spike da **forma padrão** (faixa animada always-on-bottom). Candidato **A (C#/WPF)** validado no Windows 11: **CPU 0,25% / RAM ~87 MB → PASS com folga** no risco central; always-on-bottom / no-focus / no-taskbar / multi-monitor OK. Aberto: **Win+D** (DWM cloaking → exige WorkerW, deferido ao cliente) e **soak de 8 h**. Footprint **161 MB** (self-contained WPF) é o insumo da **Ratificação de stack #1** (candidato B/Rust não implementado — A passou, sem kill). Ver `spikes/faixa-always-on-bottom/RESULTS.md`.
