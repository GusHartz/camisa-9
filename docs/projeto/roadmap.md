...
| 2.3 | **Simulação do atleta (MVP)** 🟡 **fatia A ✅ SPEC-027 (2026-07-17)** | DUAS barras persistentes (Forma e Moral) + 12 atributos evolutivos; stamina só dentro da partida (drena por físico, guia as substituições do técnico NPC, até 5/jogo). Fôlego diário cortado. *(Evolução por treino dos 4 focos ✅ SPEC-017. **Forma/Moral — fatia A ✅ SPEC-027:** as duas barras vivas e persistidas [modelo baseline+offset+decay], consumindo os seams plantados — compras [`aggregateTradeoffs`], decisões [`outcome.moral`] e comeback da lesão — aplicados evento-na-fonte; o `moral` real entra nas decisões [crise-moral deixa de ser inerte]. Só-player-store, migration `0007`, engine/goldens intocados. **Falta:** a **fatia B (SPEC-028)** — aplicar Forma/Moral à performance da partida [`effectiveAbility` + modulação no `runDailyRound`]; stamina; os 12 atributos; o FOCO diferenciado.)* |
