...
| 0.2 | **Camada de dados + seed do mundo** ✅ | Schema de ligas/clubes/atletas/temporadas; migrations versionadas. Persiste o que o spike (0.1.5) provou em memória. *(Fatiada em 5: **Fatia 1 — snapshot consultável do mundo semeado em Postgres** ✅ SPEC-013 (2026-07-16); **Fatia 2 — publicador de rodada transacional** ✅ SPEC-014 (2026-07-16): `published_round` (migration `0001`) + `publishRound` transacional (advisory lock + idempotência por PK + seam à prova de rollback), o contrato da SPEC-002 agora durável em Postgres. **Fatia 3 — viragem/rollover persistido** ✅ SPEC-021 (2026-07-17): `persistWorldTurnover` (overwrite in-place atômico + re-aplica ocupações + `turnover_report`/migration `0004` + nova âncora), disparado pelo tick no `season_complete` (`season_rolled`); a imunidade do humano entra por `immuneIds` no `advanceWorld` (engine-aware, **`world.golden.json` byte-idêntico**). **Fatia 4 — Neon (prod + branch por ambiente)** ✅ SPEC-035 (2026-07-19): driver endurecido (SSL/autosuspend + split pooled/direct) + CI Neon-branch-per-PR (fallback container secret-free); o money path é pooler-safe por construção (todos os advisory locks xact-scoped — provado por grep); ADR-002 + `neon-setup-runbook`; **sem migration** (só borda/config). **Fatia 5 — Pirâmide Elástica (R13)** ✅ SPEC-036 (2026-07-19): o mundo CRESCE — a virada expande a ~70% de ocupação humana da base (alarga a base até o teto 2×, depois novo andar), determinística por seed; a promoção cruza grupos paralelos (o playoff de acesso rico = card 2.2). Golden-safe por dispatch (1-grupo = v1 intocado) + gating (`expand?` no-op) → os 4 goldens byte-idênticos; sem migration. **A 0.2 está FECHADA em código (resta o merge da Fatia 4/PR #38).**)* |
| 0.3 | **RNG determinístico + auditoria** | Seed por temporada, log replayable de toda tick do mundo (rigor money path). |
| 0.4 | **Baseline de segurança** ✅ | Servidor HTTP (`services/api`), sessão, auth, autorização por recurso, validação de input, rate limiting, segredos em env. *(⚠️ o **servidor HTTP** não tinha item numerado em roadmap nenhum — entra aqui. **SPEC-037** (2026-07-20) entrega a 1ª fatia: `services/api` em `node:http` puro (zero dep nova) atrás do seam `RouteCtx`; sessão opaca de 256 bits com só o sha256 em `player.session` (migration `0010`), `Bearer`, idle 7d + absoluto 30d, logout que deleta a linha, cap de 10 por conta e purga 1×/tick; rotas `GET /healthz` · `POST /v1/auth/login` · `POST /v1/auth/logout`; rate limit in-process (IP 10/min + e-mail 5/min); OP-09 imposto pelo TIPO (`requireSession`), OP-11 num único serializador (`no-store` por default), CORS zero-header, body cap 8 KiB. Ver ADR-003. **SPEC-038** (2026-07-20) FECHA a baseline: o agregador `readBandState` + a rota `GET /v1/band` (contrato `BandState` congelado em `/v1`, aditivo-only; **autorização por CONSTRUÇÃO** — a rota não aceita identificador de ator, o `athleteId` só vem da sessão; `markActive` adotado como sinal de presença; rate limit em 2 camadas IP+accountId; **sem migration**; engine/goldens intocados). **Fatias futuras (não-baseline):** `POST /signup` com invite-gating, Steam auth, reset de senha, rate limit distribuído.)* |
...
| # | Spec | Entrega |
|---|------|---------|
| 1.1 | **Simulação de partida (server-side)** 🚧 | Resolução determinística por atributos+moral; resultado auditável. *(**1ª fatia — partida rica: ocorrência de lesão** ✅ SPEC-031 (2026-07-19): o `MatchResult` ganhou `events?` + a lib pura `match-events.ts` (`matchInjuries`, raro ~4%/lado, atleta do elenco + gravidade ponderada + minuto, NÃO altera o placar); o enriquecimento vive no `world-season.ts` (que tem os elencos) com um **RNG SEPARADO** (`deriveSeed(…, 'events')`) → `resolveMatch`/`simulateSeason` e os 4 goldens **byte-idênticos** (teste SCORE-NEUTRAL prova). A borda (o `services/scheduler`) injeta a lesão via `injureFromMatch` → ATIVA o arco da SPEC-026 (payoff: humano lesionado → o tick gera `lesao-volta`). **Falta:** o timeline de gols, os eventos de ESCOLHA (3.2), a lesão AFETAR o placar.)* |
| 1.2 | **Motor de temporada (rodada diária 15h)** 🚧 | Liga de 20, temporada de 38 rodadas ≈ 6 semanas; jogo diário (7/7) às 15h Brasília; **encaixe da Copa no calendário diário** (quartas intercaladas / entre temporadas / domingos de mata-mata — dentro do escopo desta spec); tabelas, rodadas, calendário do futebol; calendário e promoção/rebaixamento cientes de grupos paralelos (fundação do R13). *(**Primeira fatia — âncora diária 7/7 + orquestrador de tick + rodada-do-mundo atômica** ✅ SPEC-015 (2026-07-16): `anchor.ts` vira diário (15h todo dia; alinha o R4 FINAL), tabela `season` (âncora `start_day_index`, migration `0002`), `publishWorldRound` (todas as ligas numa tx — all-or-nothing) e `runDailyRound(db, seed, epochMs)` com boundary `season_complete` e protocolo de falha (`deferred`). **Viragem no fim da temporada ✅ SPEC-021** (Fatia 3 da 0.2): o tick agora VIRA o mundo (`season_rolled`). **Scheduler de produção ✅ SPEC-030 (2026-07-18):** o serviço greenfield `services/scheduler` com `runDailyTick(worldDb, playerDb, seed, epochMs)` — o tick diário IDEMPOTENTE que dirige todos os passes (`runDailyRound`+`moodModulator`+accrue[salário+prêmio, só em dia com rodada]+mood+decisões+recuperação+regen+vacancy), iterando `readWorldOccupations`; + `main.ts` = a borda (`Date.now()`); + o ledger `daily_ledger` (migration `0008`) que PAGA o débito de idempotência do accrue/mood. A revisão pegou 2 MAJOR de money path (accrue sem gate de `paid` → salário sem jogo + prêmio perdido no deferred-retry) → corrigidos. Engine/goldens intocados. **Cron/deploy + catch-up completo ✅ SPEC-032 (2026-07-19):** o mundo joga às 15h SOZINHO num relógio real — `dueDayIndex` (engine puro, golden intocado) + o **cursor resumível** `tick_progress` (migration `0007`) + o **loop de catch-up** no `runDailyTick` (replaya as rodadas E os passes humanos de qualquer dia perdido; same-day + multi-day; cross-season re-lê o mundo pós-viragem; um dia deferido PARA o cursor); + o **Dockerfile** (worker em container) + o **runbook** (Railway/Render cron `0,30 18-23 * * *` UTC = 15h BRT) + `.dockerignore` na raiz (segredos fora da imagem) + os `exports` que fecham a resolução em produção via `tsx`. A revisão pegou 2 MAJOR (regen órfão numa viragem falha → gate na janela de gênese `season_rolled||before_season`; `.dockerignore` inerte que vazaria segredo) + 1 MINOR (buraco silencioso no 1º tick → init `seasonStart−1`). Engine/goldens intocados. **Falta:** encaixe da Copa no calendário; multi-seed; **executar o deploy** (ação de ops); alerta ativo em falha.)* |
| 1.3 | **Ciclo de vida do NPC** | Evolução, declínio físico, aposentadoria, criação de novos atletas. |
| 1.4 | **Transferências NPC** 🚧 | Janelas, movimentação entre clubes/divisões. *(**1ª fatia — o humano como ALVO do mercado** ✅ SPEC-033 (2026-07-19): o mundo (NPC) assedia o humano por heurística (`isTransferTarget` — forte-para-o-tier), a proposta vira decisão das 18h (reusa a SPEC-025), e aceitar MOVE o humano de clube na viragem — cross-divisão, rachando o quinteto (`team_id`→NULL). `transferOccupation` = o MOVE atômico (occupy-destino + vacate-origem numa tx, sem janela órfã, na gênese, força VIVA). O `runTransfers` do engine + os 4 goldens **intocados**. A revisão adversarial pegou 5 MAJOR (divergência heurística↔move → NPC-only no pick; move-em-dobro → clear-first at-most-once; vivo vs. congelado → força viva no destino/move) → corrigidos. **Fatia 2:** o mercado NPC rico (regenera o golden), termos de contrato, mid-season.)* |
...
| # | Spec | Entrega |
|---|------|---------|
| 2.1 | **Substituição de NPC + waiting list** 🟡 | Humano assume vaga (posição/camisa/clube); vaga congela 30 dias em abandono → reverte a NPC. *(**Fatia gênese ✅ SPEC-020**: a costura `player-store`↔`world-store` + `abilityFromFocos` + o overlay `world_occupation` — o humano ocupa a vaga do NPC mais fraco na divisão de entrada e existe no mundo. **Imunidade na viragem ✅ SPEC-021**: o humano sobrevive ao rollover e sobe/desce com o clube. **Congelamento de vaga ✅ SPEC-023**: inatividade congela (e-mail "segurando sua camisa" = seam) → 30 dias → reverte a NPC (benched, carreira preservada); só-mundo, `vacancy-repo` + migration `0006` + anti-TOCTOU (writes condicionais). **Entrada imediata + waiting-list ✅ SPEC-034 (2026-07-19)**: o solo entra **IMEDIATO** (mid-season — a guarda de gênese era conservadora demais; investigação por workflow provou SAFE, money-path intocado); **teto** de humanos na entrada (cheio → **fila** FIFO); o **passe de admissão diário** herda as vagas que o revert (SPEC-023)/transferência (SPEC-033) liberam. `allowMidSeason` no `occupyNpcSlot` (só solo); migration `0008_waiting_list`; `admitOrEnqueue`/`runAdmissionPass`. **Sinal de atividade real ✅ SPEC-038:** `markActive` em `GET /v1/band` — a faixa aberta é o sinal de presença que o congelamento sempre esperou (throttle 1×/dia, best-effort). Falta: a **entrada de TIME (SPEC-035** — takeover + rebrand); a auditoria da viragem.)* |
...
| 2.3 | **Simulação do atleta (MVP)** 🟡 **Forma/Moral A+B ✅ SPEC-027+029 (2026-07-17)** | DUAS barras persistentes (Forma e Moral) + 12 atributos evolutivos; stamina só dentro da partida (drena por físico, guia as substituições do técnico NPC, até 5/jogo). Fôlego diário cortado. *(Evolução por treino dos 4 focos ✅ SPEC-017. **Forma/Moral — fatia A ✅ SPEC-027:** as duas barras vivas e persistidas [modelo baseline+offset+decay], consumindo os seams plantados — compras [`aggregateTradeoffs`], decisões [`outcome.moral`] e comeback da lesão — aplicados evento-na-fonte; o `moral` real entra nas decisões [crise-moral deixa de ser inerte]. Só-player-store, migration `0007`. **Fatia B ✅ SPEC-029:** Forma/Moral APLICADAS na partida — `effectiveAbility(base, forma, moral)` [peso 60/40, ±12%] + modulação in-memory da ability humana no `runDailyRound` [seam injetado] + recompute de `clubStrength`; base congelada da SPEC-020 intacta, engine/goldens byte-idênticos. **Falta:** stamina; os 12 atributos; o FOCO diferenciado; química; snapshot de mood por rodada [replay/auditoria].)* |
| 2.4 | **Decisões de carreira (3-5/dia)** ✅ **SPEC-025 (2026-07-17)** (o MOTOR) | Cotidiano → dramático; sem resposta = agente decide conservador às 18h. **Entregue:** o motor — geração gatilhada por estado + determinística (catálogo ABERTO, hash FNV); `answerDecision`/`resolveDeadline` (fallback conservador)/`readDecisionLog` (o log no perfil); efeitos = seam declarado (a 2.3 aplica); lock advisory + coluna `ord` (determinismo). Só-player-store (migration `0005`), engine/world-store intocados. **Fatiado:** a **proposta de transferência é REGISTRADA** (o outcome fica no log); a **execução real** (mover clube + mercado/janela/valuation) é o **card 1.4 (Transferências)** — não duplicamos o roadmap. **Deferido:** aplicar efeitos (2.3), toast/UI, wiring do scheduler. |
| 2.5 | **Lesões narrativas com arco** ✅ **SPEC-026 (2026-07-17)** | Raras, sempre recuperação → **volta por cima** (bônus, nunca punição cega). **Entregue:** o ARCO — gravidade (leve/media/grave) → dias fora → recuperando → recuperado; `injureFromMatch` = **seam da ocorrência** (a partida rica injeta — engine/goldens **intocados**), `advanceRecovery` = passe diário que fecha no prazo, `readInjuryState` = lesão ativa + `available` (o seam que o mundo lê p/ tirar o humano do jogo). **Gera decisões** reusando o motor da SPEC-025 (`injured` → template `lesao-volta`). Só-player-store (lib pura `injury.ts` + `injury-repo` sob **lock advisory + fecha-lazily** a vencida [reconcilia status↔dia], migration `0006`). Efeitos = seam (a 2.3 aplica o comeback). **Deferido:** a ocorrência real (partida rica = card 1.1/3.2), aplicar o comeback (2.3), o wiring do `advanceRecovery` no scheduler. |
...
| 4.2 | **Carreira com fim + hall de lendas (Regen)** | ~15-20 temporadas; camisa aposentada, recordes, herança de legado. **Regen (ideia do founder, 2026-07-17):** ao fim da carreira o jogador **renasce no mesmo clube** com um **reajuste de atributos** — o **gancho de FOMO/compra** (a *Carreira* vitalícia). O Regen **troca o NOME** ao renascer, para a carreira antiga permanecer no **Hall of Fame** ("lendas permanentes"). Assenta sobre a **imunidade da SPEC-021**. **✅ 1ª fatia entregue — SPEC-022 (2026-07-17):** o loop completo — encerra (**≥42 forçado / ≥25 por escolha** via `requestRegen`) → carreira antiga vira **lenda permanente** (`legend`, nome preservado) → renasce no MESMO clube, jovem (17, entrada corrigida — não herda o NPC), atributos resetados + **banco de legado** (`floor(pontos anteriores × 25%)`). Paywall = seam (`canRegen` default-permitido). Lib pura `packages/player/regen.ts` + `services/regen` (a costura pós-virada, `reassignSlot` atômico) + tabela `legend` (migration `0005`). Engine/goldens intocados. **Deferido:** paywall real (Steam), UI de rename, wiring do scheduler, regen mid-season. |
