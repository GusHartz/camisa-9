...
- **⚠️ O loop idle (SPEC-041):** o ACÚMULO de XP → pontos é **automático** — o técnico treina o foco mais baixo TODO DIA (no scheduler, até offline; não há treino manual). A **ação do jogador é DISTRIBUIR** os pontos acumulados (`POST /v1/training/spend`) — o gancho de retenção (o badge "+N pontos pra gastar" na faixa). Responder decisão / comprar / pedir regen são as outras escritas de gameplay.
